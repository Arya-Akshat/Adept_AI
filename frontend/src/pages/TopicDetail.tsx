...
const TopicDetail = () => {
  ...
  const { topicTitle, topicSummary, restorePdfId, restoreUnitIndex } = location.state || {};

  const handleBack = () => {
    // Persist restore info so Roadmap can recover the view after fresh mount
  ...
  useEffect(() => {
    const fetchExplanation = async () => {
      ...
      try {
        ...
        // Guard: check the explanation has real content
        const explanation = data.explanation;
        const hasContent =
          explanation &&
          (explanation.detailedExplanation || explanation.learningObjectives?.length || explanation.keyConcepts?.length);
            ...
            {LOADING_STEPS.map((step, i) => (
              ...
              >
                <span className={`h-2 w-2 rounded-full shrink-0 ${
                  i === stepIndex ? 'bg-orange-400 animate-pulse' : i < stepIndex ? 'bg-gray-300' : 'bg-gray-200'
                }`} />
                {step}
              </div>
            ))}
      ...
      <Layout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
          ...
          <p className="text-sm text-gray-500 max-w-sm mb-8">
            The AI couldn't generate content for <span className="font-semibold text-gray-700">{topicTitle || "this topic"}</span>. This sometimes happens with complex topics. Try again.
          </p>
          <button
    ...
    return (
      <Layout>
        <div className="flex flex-col items-center justify-center min-h-[60vh] text-center">
          ...
          <button
            onClick={handleBack}
            className="rounded-full bg-black text-white text-sm font-semibold px-5 py-2.5 hover:bg-gray-900 transition-all"
          >
            Go Back
          </button>
        </div>
